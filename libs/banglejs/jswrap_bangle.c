...
// Write the initial storage contents (and reset) if WRITE_INITIAL_STORAGE_IF_EMPTY is defined
  ...
  #else
 /* 
  Leave storage empty and run the self test code if WRITE_INITIAL_STORAGE_IF_EMPTY is not defined
...
function drawAll(){
  ...
  g.setFontArchitekt15();
  g.drawString(process.version.toUpperCase(),120,50);
...
g.drawString('Firmware: '+process.version,120,30);
var f=require('Flash');
try {
...
static void jswrap_banglejs_periph_off() {
  ...
  //jsiKill(); // RB: the call to jswrap_banglejs_kill via jswInit can cause increased power draw
  //jsvKill();
  jshKill();
